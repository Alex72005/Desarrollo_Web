...
document.querySelector('#crear').addEventListener('click', () => {
    ...
    if (nombre.length < 3 || nombre.length > 50) {
        errorNombre.textContent = "El nombre debe tener una longitud minima de 3 caracteres y maxima de 50 caracteres"
        errorNombre.style.color = "red"
        nombreCrear.style.borderColor = "red"
    }
...
nombreCrear.addEventListener('keyup', () => {
    ...
    let error = refrescarErrorNombre(nombre.length < 3 || nombre.length > 50, "El nombre debe tener una longitud minima de 3 caracteres y maxima de 50 caracteres", false)
    const myRe = /[0-9]/i
    error = refrescarErrorNombre(myRe.test(nombre), "El nombre no puede contener numeros", error)
    error = refrescarErrorNombre(nombre!=ubicacionCrear.value, "El nombre debe coincidar con el input ubicacion", error)


    // if (nombre.length < 3 || nombre.length > 50) {
    //     refrescarErrorNombre(true, "El nombre debe tener una longitud minima de 3 caracteres y maxima de 50 caracteres")
    //     // errorNombre.textContent = "El nombre debe tener una longitud minima de 3 caracteres y maxima de 50 caracteres"
    //     // errorNombre.style.color = "red"
    //     // nombreCrear.style.borderColor = "red"
    //     // nombreCrear.focus()
    // } else {
    //     refrescarErrorNombre(false)
    //     // errorNombre.textContent = ''
    //     // nombreCrear.style.borderColor = ""
    // }
})

const refrescarErrorNombre = (error, mensaje, errorAnterior) => {
    const errorNombre = document.querySelector("#errorNombre")
    if (!errorAnterior) {
        if (error) {
